...
def _cleanup_test_environment(config):
    _stop_test_rqworker()
    _cleanup_test_securedrop_dataroot(config)


def _run_in_test_environment(cmd):
...
def functional_test(args):
    ...
    # Run functional tests related to a single module
        ...
        try:
            module_path = _get_test_module_dict('functional')[args.functional]
        except KeyError:
            print('No functional test "{}" found. To list all unit tests run '
                  '`./manage.py functional -l`.'.format(args.functional))
            return 1
        else:
            cmd += module_path
